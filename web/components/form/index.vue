<template>
  <main class="my-10 px-5 container mx-auto flex-1">
    ...
    <form class="flex flex-col gap-6 p-10 mx-auto max-w-sm bg-white rounded-xl shadow-md text-gray-700">
      <FormInput label="Title" name="title" :value="form.title" @input="form.title = $event" />
      <FormInputTextArea label="Describe it" name="description" :value="form.description" @input="form.description = $event" />
      ...
      <button type="button" class="py-2 px-4 transition-colors duration-100 ease-in-out rounded-md hover:bg-white hover:text-indigo-600 bg-indigo-600 text-white" @click="createTask">
        Submit
      </button>
      <button class="bg-gray-50 py-2 px-4 rounded-md hover:text-indigo-600">
        Cancel
      </button>
  ...
  methods: {
    async createTask () {
      if (this.form.title !== '' && this.form.description !== '' && this.form.date !== '' && this.form.status !== '') {
        return await this.$api.tasks.create(this.form)
      } else {
